...
public class user_database_helper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        String createQuery = "CREATE TABLE user(userId INTEGER PRIMARY KEY, name VARCHAR(50), email VARCHAR(50),password VARCHAR(20))";
    ...
    public void insertUser(String name, String email, String password){
        ...
        if(userExists){
            ...
            ContentValues contentValues = inputContent(userId, name, email, password);

            long results = db.insert("user", null, contentValues);

            Log.i("userDbHelper", "insertUser: Inserting User Success!");
            Log.i("userDbHelper", "userId: " + userId.toString() + " name: " + name + " email: " + email + " password: " + password);
    ...
    public boolean checkRegister(String name, String email){
        SQLiteDatabase db = this.getWritableDatabase();
        String selectQuery = "SELECT * FROM user WHERE name = ? OR email = ?";
